...
impl SecretKey {
    ...
    pub fn from_bytes(bytes: impl AsRef<[u8]>) -> Result<Self> {
        Ok(SecretKey(p256::SecretKey::from_slice(bytes.as_ref())?))
    }
    ...
    pub fn sign_prehashed(
        ...
        let signing_key: p256::ecdsa::SigningKey = (&self.0).into();

        Signature(
            signing_key
                .sign_prehash_with_rng(&mut rng, prehashed_message)
                .unwrap(),
        )
    }
...
impl PublicKey {
    ...
    pub fn to_untagged_bytes(&self) -> [u8; 64] {
        self.0.to_encoded_point(false).as_ref()[1..]
            .as_ref()
            .try_into()
            .unwrap()
    }
    ...
    pub fn x(&self) -> [u8; 32] {
        self.0.to_encoded_point(false).as_ref()[1..33]
            .try_into()
            .unwrap()
    }
    ...
    /// Big-endian representation of x-coordinate.
    pub fn y(&self) -> [u8; 32] {
        self.0.to_encoded_point(false).as_ref()[33..]
            .try_into()
            .unwrap()
    }
    ...
    pub fn verify_prehashed(&self, prehashed_message: &[u8], signature: &Signature) -> bool {
        ...
        verifying_key
            .verify_prehash(prehashed_message, &signature.0)
            .is_ok()
    }
