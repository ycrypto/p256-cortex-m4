...
///
/// The internal representation is as big-endian bytes.
///
/// The intended use case is encrypted-at-rest storage.
#[derive(Clone, Zeroize)]
...
impl SecretKey {
    ...
    /// Convert endianness to construct from `SecretScalar` value.
    pub fn from_scalar(scalar: &SecretScalar) -> Self {
        let mut secret = SecretKey([0u32; 8]);
        unsafe { p256_cortex_m4_sys::p256_convert_endianness(
            &mut secret.0[0] as *mut u32 as *mut _,
            &scalar.0[0] as *const u8 as *const _,
            32,
        ) };
        secret
    }
    ...
    /// Convert endianness to obtain the inner `SecretScalar` value.
    pub fn secret_scalar(&self) -> SecretScalar {
        let mut scalar = SecretScalar([0u8; 32]);
        unsafe { p256_cortex_m4_sys::p256_convert_endianness(
            &mut scalar.0[0] as *mut u8 as *mut _,
...
impl SecretScalar {
    /// Unsafe function intended for deserialization.
    pub unsafe fn unsafe_from_bytes(bytes: &[u8; 32]) -> Self {
        Self(*bytes)
    }

    /// Unsafe function intended for serialization.
    ///
    /// Caller is responsible for keeping the data secret during processing.
    pub unsafe fn unsafe_as_bytes(&self) -> &[u8; 32] {
        &self.0
    }
}
...
impl PublicKey {
    ...
    pub fn from_xy_bytes(bytes: &[u8]) -> Result<Self> {
        if bytes.len() != 64 {
            return Err(Error);
        }
